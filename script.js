...
window.addEventListener('load', function() {
    ...
    class Player{
        ...
        draw(context){
            ...
            if(this.game.debug){
                ...
                context.stroke();
                context.beginPath();
                context.moveTo(this.collisionX, this.collisionY); // //start at
                context.lineTo(this.game.mouse.x, this.game.mouse.y); // //end at
                context.stroke();
            }
    ...
    class Obstacle {
        constructor(game){
            ...
            this.fameY = Math.floor(Math.random() * 3);
        }

        draw(context){
            context.drawImage(this.image, this.fameX * this.spriteWidth, this.fameY * this.spriteHeight, this.spriteWidth, this.spriteHeight, this.spriteX, this.spriteY, this.width, this.height);
           if(this.game.debug){
            context.beginPath();
            context.arc(this.collisionX, this.collisionY, this.collisionRadius, 0, Math.PI * 2);
            context.save();
    ...
    class Larva{
        ...
        draw(context){
            context.drawImage(this.image, this.fameX * this.spriteWidth, this.frameY * this.spriteHeight, this.spriteWidth, this.spriteHeight, this.spriteX, this.spriteY, this.width, this.height);
            if(this.game.debug){
                context.beginPath();
                context.arc(this.collisionX, this.collisionY, this.collisionRadius, 0, Math.PI * 2);
                context.save();
        ...
        update(){
            ...
            this.spriteX = this.collisionX - this.width * .5;
            this.spriteY = this.collisionY - this.height * .5 - 40;
            // // move to forest
            if (this.collisionY < this.game.topMargin){
                this.markedForDeletion = true;
                this.game.removeGameObjects();
                if(!game.gameOver) this.game.score++;
                for(let i = 0; i < 3; i++){
    ...
    class Spark extends Particle {
        update(){
            ...
            if(this.radius > .1) this.radius -= 0.05;
            if(this.radius < .2) {
                this.markedForDeletion = true;
                this.game.removeGameObjects();
            }
        ...
        render(context, deltaTime){ 
            ...
            if(this.score >= this.winningScore){
                ...
                if (this.lostHatchlings <= 5){
                    message1 = 'You won! 🏆';
                    message2 = 'We are all save now! 👼';
                } else {
                    message1 = 'GAMEOVER! ☠';
                    message2 = `Casualties: - ${this.lostHatchlings} hatchlings 😱`
                }
                context.font = '130px Helvetica';
                context.fillText(message1, this.width * .5, this.height * .5 - 20);
                context.font = '40px Helvetica';
                context.fillText(message2, this.width * .5, this.height * .5 + 30);
                context.fillText(`Final score ${this.score}. Press 'R' to restart the game!`, this.width * .5, this.height *.5 + 80);
                context.restore();
            }
        }
